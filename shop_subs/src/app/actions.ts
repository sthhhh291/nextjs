...
export const createCar = async (formData: FormData) => {
  ...
  const res = await fetch(`${baseUrl}/cars`, {
    ...
    body: JSON.stringify({ year, make, car_model, engine, vin, license, color, notes, customer_id }),
  });
...
export const createEstimate = async (formData: FormData) => {
  ...
  const res = await fetch(`${baseUrl}/cars`, {
    ...
    body: JSON.stringify({car_id,employee_id,date,hours,mileage,estimate_type}),
  });
...
export const updateEstimate = async (formData: FormData) => {
  ...
  const res = await fetch(`${baseUrl}/cars/${id}`, {
    ...
    body: JSON.stringify({id, car_id, employee_id,date,hours,mileage,estimate_type}),
  });
  console.log("updateCar response:", res);
  if (!res.ok) {
    return {
...
export const saveEstimate = async (
  prevState: { error: string | null; success: boolean; estimate: Estimate | null },
  formData: FormData,
) => {
  const id = formData.get("id") || null;
  if (id) {
...
// create a new estimate
export const createSub = async (formData: FormData) => {
  const car_id = formData.get("car_id");
  const employee_id = String(formData.get("employee_id") ?? "");
  const date = String(formData.get("date") ?? "");
  const hours = String(formData.get("hours") ?? "");
