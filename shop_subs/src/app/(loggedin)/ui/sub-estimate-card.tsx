...
export default function SubEstimateCard(params: {sub:Sub_estimate}) {
    ...
    return (
        <Card>
            ...
            {sub.labor?.map(lab => (
                <div key={lab.id}>
                    <CardContent>{lab.description} Price: {lab.price}</CardContent>
                </div>
            ))}
            ...
            {sub.parts?.map(lab => (
                <div key={lab.id}>
                    <CardContent>{lab.description} Qty: {lab.quantity} Price: {lab.price} Extended: {lab.quantity*lab.price}</CardContent>
                </div>
            ))}
            ...
            <CardContent>Subtotal: {sub.totals?.sub_total} Tax: {sub.totals?.tax} Fees: {sub.totals?.shop_fees} Total {sub.totals?.grand_total}</CardContent>
        </Card>
    )
}